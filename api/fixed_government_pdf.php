...
// Fixed government PDF generation with proper text positioning
function generateFixedGovernmentPDF($programs, $reportType, $startDate, $endDate) {
    ...
    // Overall Summary section - positioned properly
    $content .= "/F2 14 Tf\n";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Generate fixed government PDF
        $pdfContent = generateFixedGovernmentPDF($programs, $reportType, $startDate, $endDate);
        
        // Set headers for PDF download
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="fixed_government_report_' . date('Y-m-d_H-i-s') . '.pdf"');
        header('Content-Length: ' . strlen($pdfContent));
        
        echo $pdfContent;
        
    } catch (Exception $e) {
        error_log("Error generating fixed government PDF report: " . $e->getMessage());
