<?php
require_once 'config.php';

// Simple formal PDF generation without complex table drawing
function generateSimpleFormalPDF($programs, $reportType, $startDate, $endDate) {
    // Create PDF structure
    $pdf = "%PDF-1.4\n";
    
    // Add font dictionary
    $pdf .= "1 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Type /Font\n";
    $pdf .= "/Subtype /Type1\n";
    $pdf .= "/BaseFont /Helvetica\n";
    $pdf .= ">>\n";
    $pdf .= "endobj\n";
    
    $pdf .= "2 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Type /Font\n";
    $pdf .= "/Subtype /Type1\n";
    $pdf .= "/BaseFont /Helvetica-Bold\n";
    $pdf .= ">>\n";
    $pdf .= "endobj\n";
    
    // Catalog
    $pdf .= "3 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Type /Catalog\n";
    $pdf .= "/Pages 4 0 R\n";
    $pdf .= ">>\n";
    $pdf .= "endobj\n";
    
    // Pages
    $pdf .= "4 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Type /Pages\n";
    $pdf .= "/Kids [5 0 R]\n";
    $pdf .= "/Count 1\n";
    $pdf .= ">>\n";
    $pdf .= "endobj\n";
    
    // Create page content with simple text positioning
    $content = "BT\n";
    
    // Header
    $content .= "/F2 20 Tf\n";
    $content .= "50 750 Td\n";
    $content .= "(KEDAH STATE GOVERNMENT) Tj\n";
    $content .= "0 -30 Td\n";
    $content .= "/F2 16 Tf\n";
    $content .= "(PROGRAM MANAGEMENT SYSTEM) Tj\n";
    $content .= "0 -40 Td\n";
    $content .= "/F2 18 Tf\n";
    $content .= "(" . strtoupper($reportType) . " PROGRAMS REPORT) Tj\n";
    $content .= "0 -50 Td\n";
    
    // Report info
    $content .= "/F1 10 Tf\n";
    $content .= "(Generated: " . date('d-m-Y H:i:s') . ") Tj\n";
    $content .= "0 -30 Td\n";
    
    // Summary section
    $totalBudget = array_sum(array_column($programs, 'budget'));
    $content .= "/F2 14 Tf\n";
    $content .= "(EXECUTIVE SUMMARY) Tj\n";
    $content .= "0 -25 Td\n";
    $content .= "/F1 12 Tf\n";
    $content .= "(Total Programs: " . count($programs) . ") Tj\n";
    $content .= "0 -20 Td\n";
    $content .= "(Total Budget Allocation: RM " . number_format($totalBudget, 0) . ") Tj\n";
    $content .= "0 -40 Td\n";
    
    // Table header
    $content .= "/F2 14 Tf\n";
    $content .= "(DETAILED PROGRAM LISTING) Tj\n";
    $content .= "0 -30 Td\n";
    
    // Simple table header
    $content .= "/F2 10 Tf\n";
    $content .= "50 550 Td\n";
    $content .= "(No.  Program Title                    Status      Budget RM    Date) Tj\n";
    $content .= "0 -25 Td\n";
    
    // Simple separator line
    $content .= "50 520 Td\n";
    $content .= "(----------------------------------------------------------------) Tj\n";
    $content .= "0 -25 Td\n";
    
    // Table data rows - simple text positioning
    foreach ($programs as $index => $program) {
        $content .= "/F1 9 Tf\n";
        
        // Create a formatted line for each program
        $title = isset($program['title']) ? substr($program['title'], 0, 30) : 'No Title';
        $status = isset($program['status']) ? strtoupper($program['status']) : 'UNKNOWN';
        $budget = isset($program['budget']) ? number_format($program['budget'], 0) : '0';
        $date = isset($program['created_at']) ? date('d/m/Y', strtotime($program['created_at'])) : 'N/A';
        
        // Pad strings to create table-like appearance
        $no = str_pad(($index + 1) . ".", 4);
        $title = str_pad($title, 35);
        $status = str_pad($status, 12);
        $budget = str_pad($budget, 12);
        
        $line = $no . " " . $title . " " . $status . " " . $budget . " " . $date;
        $content .= "({$line}) Tj\n";
        $content .= "0 -20 Td\n";
    }
    
    // Footer
    $content .= "0 -30 Td\n";
    $content .= "/F1 10 Tf\n";
    $content .= "(Generated by Kedah State Government Program Management System) Tj\n";
    $content .= "0 -15 Td\n";
    $content .= "(For official use only - This document is confidential) Tj\n";
    $content .= "0 -15 Td\n";
    $content .= "(Report Type: " . strtoupper($reportType) . " Programs) Tj\n";
    
    $content .= "ET\n";
    
    // Page object
    $pdf .= "5 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Type /Page\n";
    $pdf .= "/Parent 4 0 R\n";
    $pdf .= "/MediaBox [0 0 612 792]\n";
    $pdf .= "/Resources <<\n";
    $pdf .= "/Font <<\n";
    $pdf .= "/F1 1 0 R\n";
    $pdf .= "/F2 2 0 R\n";
    $pdf .= ">>\n";
    $pdf .= ">>\n";
    $pdf .= "/Contents 6 0 R\n";
    $pdf .= ">>\n";
    $pdf .= "endobj\n";
    
    $pdf .= "6 0 obj\n";
    $pdf .= "<<\n";
    $pdf .= "/Length " . strlen($content) . "\n";
    $pdf .= ">>\n";
    $pdf .= "stream\n";
    $pdf .= $content;
    $pdf .= "endstream\n";
    $pdf .= "endobj\n";
    
    $pdf .= "xref\n";
    $pdf .= "0 7\n";
    $pdf .= "0000000000 65535 f \n";
    $pdf .= "0000000009 00000 n \n";
    $pdf .= "0000000058 00000 n \n";
    $pdf .= "0000000115 00000 n \n";
    $pdf .= "0000000172 00000 n \n";
    $pdf .= "0000000250 00000 n \n";
    $pdf .= "0000000350 00000 n \n";
    $pdf .= "trailer\n";
    $pdf .= "<<\n";
    $pdf .= "/Size 7\n";
    $pdf .= "/Root 3 0 R\n";
    $pdf .= ">>\n";
    $pdf .= "startxref\n";
    $pdf .= (strlen($pdf) - 1) . "\n";
    $pdf .= "%%EOF\n";
    
    return $pdf;
}

// Handle the report generation
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $pdo = getConnection();
        
        $input = json_decode(file_get_contents('php://input'), true);
        $reportType = $input['reportType'] ?? 'all';
        $startDate = $input['startDate'] ?? '';
        $endDate = $input['endDate'] ?? '';
        
        // Build the SQL query based on report type
        $whereConditions = [];
        $params = [];
        
        if ($reportType === 'approved') {
            $whereConditions[] = "p.status = 'approved'";
        } elseif ($reportType === 'rejected') {
            $whereConditions[] = "p.status = 'rejected'";
        }
        
        if ($startDate) {
            $whereConditions[] = "p.created_at >= ?";
            $params[] = $startDate . ' 00:00:00';
        }
        
        if ($endDate) {
            $whereConditions[] = "p.created_at <= ?";
            $params[] = $endDate . ' 23:59:59';
        }
        
        $whereClause = '';
        if (!empty($whereConditions)) {
            $whereClause = " WHERE " . implode(" AND ", $whereConditions);
        }
        
        $sql = "
            SELECT p.*, u.name as user_name, u.role as user_role,
                   a.name as approver_name, r.name as rejector_name
            FROM programs p 
            LEFT JOIN users u ON p.user_id = u.id 
            LEFT JOIN users a ON p.approved_by = a.id
            LEFT JOIN users r ON p.rejected_by = r.id
            {$whereClause}
            ORDER BY p.created_at DESC
        ";
        
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        $programs = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Generate simple formal PDF
        $pdfContent = generateSimpleFormalPDF($programs, $reportType, $startDate, $endDate);
        
        // Set headers for PDF download
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="' . $reportType . '_simple_formal_report_' . date('Y-m-d_H-i-s') . '.pdf"');
        header('Content-Length: ' . strlen($pdfContent));
        
        echo $pdfContent;
        
    } catch (Exception $e) {
        error_log("Error generating simple formal PDF report: " . $e->getMessage());
        http_response_code(500);
        echo json_encode(['error' => 'Failed to generate PDF report']);
    }
} else {
    http_response_code(405);
    echo json_encode(['error' => 'Method not allowed']);
}
?> 